...
void add(int l, int r, int x, int L, int R, ll val) {
    if(L <= l && r <= R) {
        lazy[x] += val;
        sum[x] += val * (r - l + 1);
    }
        ...
        down(x, l, r);
        int mid = (r - l) / 2 + l;
        if(mid >= L) add(l, mid, lson, L, R, val);
        if(mid + 1 <= R) add(mid + 1, r, rson, L, R, val);
        updata(x);
     }
}
ll query(int l, int r, int x, int L, int R) {
    ...
    if(L <= 1 && r <= R) {
        ...
        if(mid + 1 <= R) ans += query(mid + 1, r, rson, L, R);
    }
    return ans;
}
int main()
